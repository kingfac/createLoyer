...
<table style=" width:100%; font-size: 0.7em; font-weight: bold; color:rgb(46, 131, 211); padding-top:10px">
    <tr  style="">
        ...
        <td style="text-align:right;" colspan="3">
            <h4>Kin, le {{$lelo}}</h4>
            <h4 class="p-2 bg-gray-200" style="width: 100%;">Montant :  <b style="padding: 5px; background-color:rgb(98, 172, 241); width:100%;color:white">{{ $records[1]->montant  }} $ </b></h4>
            <h4>Loyer de :  {{$records[1]->mois}}-{{$records[1]->annee}}</h4>
            @php
                $nom = User::find($records[0]['users_id']);
            @endphp
            <h4>Intervenant : {{$nom->name}}</h4>
...
<div class="text-center b-2 bg-gray-500 mb-2" style="font-size: 1em;padding:10px; color:rgb(46, 131, 211)">RECU LOYER, PAIEMENT ANTICIPATIF N° {{ $records[1]->id }}</div>

<table class="w-full mb-2" style="font-size: 0.7em; " style=" color:rgb(46, 131, 211)">
    <tr  class="text-bold">
        <td ><b>Recu de :</b> <span style=" text-decoration:underline;"><b>{{ $records[1]->locataire->noms }}</b>, Galerie: <b>{{ $records[1]->locataire->occupation->galerie->nom }}-{{$records[1]->locataire->occupation->galerie->num}}</b></span></td>
        {{-- <td ></td> --}}
    </tr>
    <tr style="margin-top: 50px; padding-top:10px; width:20px; height:20" class=" py-6">
        <td><b>Somme de (en toutes lettres)</b> <b style="padding: 5px; background-color:gray; width:100%;"></b>  </td>    
    ...
    <tr style="font-size: 1.1em; " class="text-bold">
        <td colspan="" style="">
            <div style="padding: 12px; background-color: rgb(135, 190, 241); width:95%">
                <p style=" color:white; font-size:0.9em; text-transform: capitalize;">{{SpellNumber::value(intval($records[1]->montant))->locale('fr')->toLetters()}} dollars américains.</p>
            </div>
            <b></b>
        </td>
    ...
    <tr>
        <td style="width:100%"><b>Pour :</b>{{$records[1]->observation ?? ' Loyer du mois de (d\') '.$records[1]->mois}}, 
